...
DatePicker.prototype	= {
 ...
 _initCurrentDate_ : function () {
  /* check if value in field is proper, set to today */
  this._current_date	= $F(this._relative);
  if ( !this._date_regexp.test(this._current_date) ) {
   var now	= new Date();
   var day	= this._leftpad_zero(now.getDate(), 2);
   var mon	= this._leftpad_zero(now.getMonth(), 2);
   /* depending on language not presented the same way */
   if ( this._language == 'en' )
    this._current_date	= mon+'/'+day+'/'+now.getFullYear();
   else 
 ...
 cierra : function () {
 	Event.stopObserving(this._relative, 'click', this.click);
 },
 /**
 ...
 click		: function () {
  ...
  if (this._id_datepicker!=null && this._id_datepicker!=undefined && !this._isPositionned && this._relativePosition ) {
  ...
  $(this._id_datepicker).style.display = 'block';
 },
 /**
