...
class MiniXMLElement extends MiniXMLTreeComponent {
		...
		*/
		if ($this->xavoidLoops || (MINIXML_AUTOSETPARENT > 0) )
		{
			if ($this->xparent == $child)
			{
				...
				$cname = $child->name();
				return _MiniXMLError('MiniXMLElement::insertChild() Tryng to append parent $cname as child of '. $this->xname );
			}
			$child->parent($this);
		}
		...
		*/
		if ($this->xavoidLoops || (MINIXML_AUTOSETPARENT > 0) )
		{
			if ($this->xparent == $child)
			{
				...
				$cname = $child->name();
				return _MiniXMLError('MiniXMLElement::appendChild() Tryng to append parent $cname as child of '. $this->xname );
			}
			$child->parent($this);
		}
	...
	*/
	...
	function _validateChild (&$child)
	{
		...
		{
			_MiniXMLLog('MiniXMLElement::_validateChild() Trying to append a child ($cname) that already has a parent set '. 'while avoidLoops is on - aborting');
			return 0;
		}
