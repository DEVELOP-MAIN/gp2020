...
if(isset($_GET['c'])) $idcliente = $_GET['c']; else $idcliente = $_POST['idclnt'];

//incio el objeto
$clnt = new cliente();
if($clnt->select($idcliente))
	$nombre_sup = $clnt->getRazon_social();
else
	$nombre_sup = '';
...
if(isset($_POST['proceso']) && $_POST['proceso']=='S')
{
	if(isset($_FILES['frm_alta_csv']) && $_FILES['frm_alta_csv']['tmp_name']!='')
	{
		...
		while(!feof($file))
		{
			...
			if(count($_arr_content)>=4) 
			{
				$PUNTOS								=	sanear_string(trim($_arr_content[0]));
				$FECHA									=	trim($_arr_content[1]);
				$OBSERVACIONES	=	sanear_string(trim($_arr_content[2]));
				$MOTIVO								=	sanear_string(trim($_arr_content[3]));
								...
								<tr>
									...
									<td height="150" valign="top" bgcolor="#FFFFFF">
										<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0">
											<tr>
												<td height="25" valign="bottom" class="textoMarron14sinhover"><strong>IMPORTAR PUNTOS PARA <?php echo $nombre_sup;?></strong></td>
                          ...
                          <div class="ayuda">
                          	<table>
															<tr>
																<td>&#160;&#160;&#160;</td>
																<td>&#160;&#160;&#160;</td>
																<td>Desde este modulo se procesa el archivo de puntos para "<?php echo $nombre_sup;?>".<br /><strong>La primer fila del archivo sera ignorada dado que se espera contenga los nombres de cada columna.</strong><br />El mismo debe contener la informaci&#243;n de los puntos bajo una estructura determinada. Cada l&#237;nea deber&#225; tener los siguientes campos siempre separados por <strong>punto y coma</strong>.<br /><strong>FORMATO:</strong>PUNTOS; FECHA; OBSERVACIONES; MOTIVO</td>
															</tr>
														</table>
                          </div>
													<?php }?>
													...
													<form name="formdatos" id="formdatos" action="importacion_puntos.php" method="POST">
														<input type="hidden" id="idclnt" name="idclnt" value="<?php echo $idcliente;?>">
