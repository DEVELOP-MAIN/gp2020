...
$sc = new socio();
if(!$sc->select($idsocio))
	exit;
else {
	$socio = $sc->getNombre().' '.$sc->getApellido();
	$movimientos = array();
	$movimientos = $sc->getMovimientos('asc','');
	...
	<html>
		...
		<body>
			<table border="1" cellspacing="2" cellpadding="2">
				<tr>
					<td colspan="3" align="center"><font face="arial" size="2"><strong>LISTADO DE MOVIMIENTOS DE '.$socio.'</strong></td>
				</tr>';
	$data .= '<tr>';
		$data .= '<td bgcolor="#CCCCCC"><strong>FECHA</strong></td>';
		$data .= '<td bgcolor="#CCCCCC"><strong>CONCEPTO</strong></td>';
		$data .= '<td bgcolor="#CCCCCC"><strong>MILLAS</strong></td>';
	$data .= '</tr>';
