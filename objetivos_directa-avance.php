...
$vendedores_lin = $lst->getObjetivosSocios(intval($mes),intval($anio),'','', 'vendedor','so.apellido, pu.linea ASC');
						...
						<div class="col-lg-12">
							...
							<table class="table table-sm">
								...
								for($i=0;$i<$nro_vend_lin;$i++) {
									...
									$color = dameColorSegunAvance($cumplimiento_por);
								?>
								<?php if($vendedor_anterior != $apellido && $i>0) {?>
								<tr>
									<td colspan="5" class="gp-separador"></td>
								</tr>
								<?php } ?>
